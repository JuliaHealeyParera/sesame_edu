...
print(kable(presence_tbl, caption = "Predictors Kept Across Final Models"))

```
[not sure if we wanna keep this format... taking up a lot of space but it's clearer to look at this way though]

This table summarizes the predictors kept in each of the six final beta regression models following the lasso-based variable-selection procedure. Across all outcomes, the main effects of age, site, viewing frequency (viewcat), setting, viewing encouragement, and baseline vocabulary were consistently retained, indicating that these factors play a broadly influential role in children’s learning gains.

Several interaction terms also appeared in all or nearly all models, ... [elaborate !!!]

Overall, this retention pattern shows that children’s improvement is shaped not only by individual predictors but also by how those predictors interact, justifying the inclusion of interaction terms and supporting a nuanced interpretation of Sesame Street’s educational impact.
...
ggplot(sesame_edit, aes(x = age, y = improve_let, color = viewcat)) +
  ...
  labs(
    title = "Age × Viewing Frequency on Letter Improvement",
    x = "Age (months)",
    y = "Letter Improvement (%)",
    color = "Viewing\nFrequency"
  ) +
...
```
[Table shows `age:viewcat` is kept in all six final models, which is why I plotted it.] The effect of age depends on how often the child watches Sesame Street. GOOD!!! [pls elaborate thank youu]
...
ggplot(sesame_edit, aes(x = viewcat, y = improve_let, fill = setting)) +
  ...
  labs(
    title = "Viewing Frequency × Setting on Letter Improvement",
    x = "Viewing Frequency",
    y = "Letter Improvement (%)",
    fill = "Viewing\nSetting"
  ) +
...
```
The effect of viewing frequency is different in school vs home settings. [elaborate!!!] ex: In lower viewing frequencies, home vs school differences are small.
At moderate frequencies, differences widen. When watching at home, continues to improve as viewing frequency increases but not the same pattern for school.

```{r}
# sex & setting interaction effect
ggplot(sesame_edit, aes(x = setting, y = improve_let, fill = sex)) +
  geom_boxplot(outlier.color = "dimgrey", alpha = 0.75) +
  scale_fill_manual(values = c(
    "male"   = "lightskyblue3",
    "female" = "thistle3"
  )) +
  labs(
    title = "Setting × Sex on Letter Improvement",
    x = "Viewing Setting",
    y = "Letter Improvement (%)",
    fill = "Sex"
  ) +
...
```
tbh not sure if this is useful so can consider deleting
...
ggplot(sesame_edit, aes(x = viewcat, y = improve_let, fill = sex)) +
  ...
  labs(
    title = "Sex × Viewing Frequency on Letter Improvement",
    y = "Letter Improvement (%)",
    x = "Viewing Frequency",
    fill = "Sex"
  ) +
...
```
Across all four plotted interactions, the patterns in the observed data mirror the model’s retained interaction terms, strengthening confidence that the fitted models capture meaningful variation in how demographic and contextual factors shape early learning outcomes.
